...
def load_weather_folder(folder: str | Path) -> pd.DataFrame:
    ...
    if not files:
        raise FileNotFoundError(f"No weather files found in {folder} matching 'df_weather_weekly_*.csv'")
    ...
    for f in files:
        ...
        keep_cols = [c for c in [
            "tmean","humidity","rain","dewpoint","wind10","wind100","winddir10","gust10",
            "soil_temp_0_7cm","soil_moist_0_7cm"
        ] if c in dfw.columns]
    ...
    if not frames:
        raise RuntimeError("Weather files were found, but none matched expected naming patterns.")
    long_wx = pd.concat(frames, ignore_index=True)
    # drop NA dates
    long_wx = long_wx.dropna(subset=["week_start_date"])
    # sort
    cols_order = ["geo_id","geo_name","week_start_date"] + [c for c in long_wx.columns if c not in {"geo_id","geo_name","week_start_date"}]

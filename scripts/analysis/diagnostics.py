...
- MAE by geo_id â†’ artifacts/tables/mae_by_geo_<tag>.csv
- Error vs magnitude (y_true deciles) â†’ artifacts/tables/error_by_quantile_<tag>.csv
- Calibration table (pred deciles) â†’ artifacts/tables/calibration_<tag>.csv
...
def main():
    ...
    cal.plot()
    plt.title(f"Calibration (mean obs vs pred) â€” {args.tag}")
    plt.savefig(fig1, bbox_inches="tight")
    plt.close()

    print(f"[diag] MAE by geo â†’ {out1}")
    print(f"[diag] Error vs magnitude â†’ {out2}")
    print(f"[diag] Calibration â†’ {out3}")
    print(f"[diag] Calibration plot â†’ {fig1}")

"""Persistence baseline: y_pred(geo, t) = cases(geo, origin_week).
Writes:
  artifacts/forecasts/persistence_<FOLD>.parquet
  artifacts/tables/metrics_persistence_<FOLD>.csv
"""
...
def main():
    ...
    XY["model"] = "persistence"

    out_fore = paths["forecasts"] / f"persistence_{args.fold}.parquet"
    ensure_dir(out_fore.parent)
    XY.to_parquet(out_fore, index=False)

    m = metrics(XY)
    m.insert(0, "source", f"metrics_persistence_{args.fold}.csv")

    out_tbl = paths["tables"] / f"metrics_persistence_{args.fold}.csv"
    ensure_dir(out_tbl.parent)
    m.to_csv(out_tbl, index=False)

    print(f"[persistence] forecasts → {out_fore} ({len(XY):,} rows)")
    print(f"[persistence] metrics   → {out_tbl}")
